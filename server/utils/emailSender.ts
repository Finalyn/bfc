...
const FROM_EMAIL = process.env.SMTP_USER || "noreply@finalyn.com";
    ...
    html: `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff;">
        ...
        <div style="padding: 25px 20px;">
          <p style="font-size: 15px; color: #333;">Bonjour <strong>${order.responsableName}</strong>,</p>
          ...
          <div style="background: #f8fafc; padding: 18px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #2563eb;">
            <table style="width: 100%; border-collapse: collapse;">
              <tr><td style="padding: 6px 0; color: #666; font-size: 13px;">Référence</td><td style="padding: 6px 0; text-align: right; font-weight: 600; color: #2563eb; font-size: 13px;">${order.orderCode}</td></tr>
              ...
              <tr><td style="padding: 6px 0; color: #666; font-size: 13px;">Commercial</td><td style="padding: 6px 0; text-align: right; color: #333; font-size: 13px;">${order.salesRepName}</td></tr>
          ...
          <div style="background: #f8fafc; padding: 18px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #2563eb;">
            ...
            <p style="margin: 0; color: #555; font-size: 13px; line-height: 1.5;">
              ${order.livraisonEnseigne}<br/>${order.livraisonAdresse}<br/>${order.livraisonCpVille}
            </p>
          </div>

          <p style="font-size: 13px; color: #555; line-height: 1.5;">
            Vous trouverez votre bon de commande en pièce jointe.<br/>
            Pour toute question, contactez votre commercial <strong>${order.salesRepName}</strong>.
          </p>
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #fff;">
        <div style="padding: 20px; border-bottom: 3px solid #2563eb;">
          ...
          <p style="margin: 5px 0 0 0; color: #2563eb; font-size: 15px; font-weight: 600;">${order.orderCode}</p>
        </div>
        ...
        <div style="padding: 20px;">
          <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            ...
            <tr style="border-bottom: 1px solid #eee;"><td style="padding: 8px 0; color: #666;">Commercial</td><td style="padding: 8px 0; text-align: right;">${order.salesRepName}</td></tr>
          ...
          <div style="margin: 20px 0; padding: 15px; background: #f8fafc; border-left: 3px solid #2563eb; border-radius: 4px;">
            ...
            <p style="margin: 0; font-size: 13px;"><strong>${order.livraisonEnseigne}</strong> — ${order.responsableName}</p>
            <p style="margin: 3px 0 0 0; font-size: 12px; color: #666;">${order.responsableTel} · ${order.responsableEmail}</p>
          </div>
          ...
          <div style="margin: 15px 0; padding: 15px; background: #f8fafc; border-left: 3px solid #2563eb; border-radius: 4px;">
            ...
            <p style="margin: 0; font-size: 13px;">${order.livraisonAdresse}, ${order.livraisonCpVille}</p>
            ${order.livraisonHoraires ? `<p style="margin: 3px 0 0 0; font-size: 12px; color: #666;">Horaires : ${order.livraisonHoraires}</p>` : ''}
          ...
          <div style="margin: 15px 0; padding: 15px; background: #f8fafc; border-left: 3px solid #2563eb; border-radius: 4px;">
            ...
            <p style="margin: 0; font-size: 13px;">${order.facturationRaisonSociale}</p>
            <p style="margin: 3px 0 0 0; font-size: 12px; color: #666;">${order.facturationAdresse}, ${order.facturationCpVille}</p>
            <p style="margin: 3px 0 0 0; font-size: 12px; color: #666;">Paiement : ${order.facturationMode}${order.facturationRib ? ` — RIB : ${order.facturationRib}` : ''}</p>
          </div>
          ...
          <div style="margin: 15px 0; padding: 15px; background: #f8fafc; border-left: 3px solid #2563eb; border-radius: 4px;">
            ...
            <p style="margin: 0; font-size: 13px; color: #333;">${order.remarks}</p>
          </div>
          ` : ""}
        </div>
